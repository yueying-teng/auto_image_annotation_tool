...
def format_line(image_path, label_confidence_dic):
  """
  format the predicted label and probability pairs for each image so that they can be writen to one csv cell
  Args:
    image_path: path of the input image in string 
    label_confidence_dic: dictioanry of predicted label names and prediction probabilities 
  Returns:
    fomat_line: a string with comma seperated columns 
  """
  line = [(label, str(np.round(v, 3))) for label, v in label_confidence_dic.items()]
  format_line = image_path + "," + " ".join(["{} {}".format(pair[0], pair[1]) for pair in line]) + "\n"
...
def main(args):
  ...
  with g.as_default():
    ...
    folder_name = FLAGS.image_folder_path.split('/')[-1]
    csv_name = folder_name + '.csv'
    output_path = os.path.join('/inception/output', csv_name)

    with open(output_path, 'w') as f:
      f.write('image_id,label_confidence_pair\n')
      
      # Run the evaluation on the images
      for i in range(len(img_dir)):
        image_path = img_dir[i]
        if not os.path.exists(image_path):
          tf.logging.fatal('Input image does not exist %s', image_path[0])
        ...
        for l in format_line(image_path, label_confidence_dic):
          f.write(l)
...
if __name__ == '__main__':
  ...
  # parser.add_argument('--output_path', type=str, default='/inception/scripts/ouput_file.csv',
  #                     help='location to save the output csv.')
